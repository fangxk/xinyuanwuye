...
class Submi extends Api
{
    ...
    public function SubmitQuestion()
    {
        ...
        $radio = empty($this->request->param("radio")) ? array() : explode(',', substr(htmlspecialchars_decode($this->request->param("radio")), 1, -1));
        // $this->request->param("radio");
        $checkbox = empty($this->request->param("checkbox")) ? array() : explode(',', substr(htmlspecialchars_decode($this->request->param("checkbox")), 1, -1));
        //$this->request->param("checkbox");
        $content = empty($this->request->param("content")) ? array() : explode(',', substr(htmlspecialchars_decode($this->request->param("content")), 1, -1));
        //$this->request->param("content");
        ...
        for ($i = 0; $i < count($newdata); $i++) {
            $new = explode("-", str_replace('"', '', $newdata[$i]));
            ...
            $data["project_name"] = $userinfo["project_name"];
        ...
        $voteuser = array("uid" => $uid, "voteid" => $voteid, "project_name" => $userinfo["project_name"], "avatar" => $userinfo["avatar"], "relaname" => $userinfo["name"], "mobile" => $userinfo["mobile"], "nickname" => $userinfo["nickname"], "house_name" => $userinfo['house_name'], "pk_client" => $userinfo['pk_client'], "pk_house" => $userinfo['pk_house'], "pk_project" => $userinfo['pk_project'], "createtime" => time());
        //print_r($answers);die;
    ...
    public function SubmitActivity()
    {
        ...
        if ($active) {
            ...
            //验证用户是否完善信息
            $data = array("uid" => $uid, "nickname" => $userinfo['nickname'], "project_name" => $userinfo['project_name'], "house_name" => $userinfo['house_name'], "pk_client" => $userinfo['pk_client'], "pk_house" => $userinfo['pk_house'], "pk_project" => $userinfo['pk_project'], "relaname" => $userinfo['name'], "mobile" => $userinfo['mobile'], "aid" => $id, "avatar" => $userinfo['avatar'], "address" => $userinfo['house_name'], "createtime" => time());
    ...
    public function SubmitVote()
    {
        ...
        if ($vote) {
            ...
            $data = array("uid" => $uid, "option_ids" => $optionid, "nickname" => $userinfo['nickname'], "relaname" => $userinfo['name'], "mobile" => $userinfo['mobile'], "aid" => $id, "avatar" => $userinfo['avatar'], "address" => $userinfo['house_name'], "createtime" => time());
