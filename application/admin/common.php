...
/*if(!function_exists("digui")){
    //递归同步组织结构数据  00AC523220002A758A96
    function digui($arr,$pid=0)
    {
        $res = array();
        foreach ($arr as $k => $v) {
            if ($v['parentguid'] == $pid) {
                $v['child'] = digui($arr, $v['recguid']);
                $res[] = $v;
            }
        }
        return $res;
